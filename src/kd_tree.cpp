...
// From J. Frederico Carvalho's k-d tree implementation at https://github.com/crvs/KDTree
...
KDTree::KDTree(pointVec point_array) {
    ...
    for (size_t i = 0; i < point_array.size(); i++) {
        pts_.push_back(pointIndex(point_array.at(i), i));
    }

    auto begin = pts_.begin();
    auto end = pts_.end();

    size_t length = pts_.size();
...
const pointIndexArr& KDTree::get_points() {return pts_;} // rzfeng
